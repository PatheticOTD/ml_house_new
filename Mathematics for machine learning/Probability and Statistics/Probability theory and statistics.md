...
Поэтому условное распределение определяется, как $p(x_1|x_{2}= -1) = N(0.6, 0.1)$ 
...
Априорное распределение и апостериорное распределение принадлежат к одному и тому же семейству.

##### Sufficient Statistics
Вспомним, что статистика случайной переменной есть детерминистическая функция.

Теорема Фишера-Неймана: Пусть X имеет pdf p(x|$\theta$). Тогда статистика $\phi(x)$ будет считаться достаточной для тета тогда и только тогда, когда p(x|$\theta$) может быть написана как $$
p(x|\theta) = h(x)g_{\theta}(\phi(x))
$$где h(x) - распределение независимое от тета и $g_{\theta}$ вмещает в себя все зависимости от тета через статистику фи.

#### Law Of Total Probability
Это теорема, которая в своём дискретном случае утверждает, что если у нас есть набор событий {${B_{n}:n = 1, 2, 3, ...}$}, которые взаимоисключающи и формируют полную группу, тогда для события A
$$
P(A) = \sum\limits_{n}P(A\cap B_{n)}= \sum\limits_{n}P(A|B_n)P(B_n)
$$
